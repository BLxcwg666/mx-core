...
export class CronTaskScheduler {
  ...
  async scheduleCleanLogFile() {
    this.logger.log('Scheduling cleanLogFile task')
    const result = await this.cronTaskService.createCronTask(
      CronTaskType.CleanLogFile,
    )
    this.logger.log(
      `cleanLogFile task ${result.created ? 'created' : 'already exists'}: ${result.taskId}`,
    )
  }
