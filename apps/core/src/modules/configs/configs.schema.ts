...
export const S3OptionsSchema = section('S3 对象存储设置', {
  ...
  pathStyleAccess: field.toggle(z.boolean().optional(), '路径风格访问', {
    description:
      '启用路径风格访问（Path-style），适用于 MinIO 等兼容 S3 的服务',
  }),
})
export class S3OptionsDto extends createZodDto(S3OptionsSchema) {}
export type S3OptionsConfig = z.infer<typeof S3OptionsSchema>

// ==================== Backup Options ====================
export const BackupOptionsSchema = section('备份设置', {
  enable: field.toggle(z.boolean().optional(), '开启自动备份到 S3', {
    description: '需要先配置 S3 对象存储设置',
  }),
  path: field.plain(z.string().optional(), '备份文件路径', {
    description:
...
// ==================== Image Bed Options ====================
export const ImageBedOptionsSchema = section('图床设置', {
  enable: field.toggle(z.boolean().optional(), '启用 S3 图床', {
    description:
      '启用后，编辑器上传的图片将存储到 S3；需要先配置 S3 对象存储设置',
  }),
  path: field.plain(z.string().optional(), '图片存储路径', {
    description:
      '支持占位符：{Y}年4位 {y}年2位 {m}月 {d}日 {h}时 {i}分 {s}秒 {timestamp}时间戳 {uuid} {md5} {md5-16} {str-N}随机字符串 {filename}原文件名',
  }),
  allowedFormats: field.plain(z.string().optional(), '允许的图片格式', {
    description: '逗号分隔，例如：jpg,jpeg,png,gif,webp',
  }),
  maxSizeMB: field.number(
    z.preprocess(
      (val) => (val ? Number(val) : val),
...
// ==================== MeiliSearch Options ====================
export const MeiliSearchOptionsSchema = section('MeiliSearch 设置', {
  enable: field.toggle(z.boolean().optional(), '启用 MeiliSearch', {
    description:
      '本地全文搜索引擎，需要在 docker-compose 中启用 meilisearch 服务',
  }),
  indexName: field.plain(z.string().optional(), '索引名称', {
    description: '搜索索引名称，默认为 mx-space',
  }),
  searchCacheTTL: field.number(
    z.preprocess(
      (val) => (val ? Number(val) : val),
      z.number().int().min(60).optional(),
    ),
    '搜索缓存时间（秒）',
    { description: '搜索结果缓存时间，默认 300 秒' },
  ),
})
export class MeiliSearchOptionsDto extends createZodDto(
  MeiliSearchOptionsSchema,
) {}
export type MeiliSearchOptionsConfig = z.infer<typeof MeiliSearchOptionsSchema>
...
export const configSchemaMapping = {
  ...
  meiliSearchOptions: MeiliSearchOptionsSchema,
